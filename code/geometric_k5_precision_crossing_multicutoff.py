#!/usr/bin/env python3
"""
geometric_k5_precision_crossing.py  (v4: multi-cutoff)

Precision wrapper for geometric_k5_hybrid.py.

This version accepts TWO OR MORE exact/PNT cutoffs and either TWO or THREE FFT grids.
...
Recommended run
---------------
    python geometric_k5_precision_crossing_multicutoff.py \
      --bracket 1e11,2e11 \
      --cutoffs 2e8,4e8,8e8 \
...
def main() -> None:
    ...
    ap.add_argument(
        "--cutoffs",
        default="2e8,4e8,8e8",
        help="two or more exact-prime cutoffs, e.g. 2e8,4e8,8e8",
    )
    ap.add_argument(
        "--grids",
    ...
    if len(cutoffs) < 2:
        raise SystemExit("--cutoffs must contain at least two values")
    if len(grids) not in (2, 3):
    ...
    print("\nSummary")
    print("-------")
    print(f"best estimate (largest cutoff): n ~= {central:,.0f}")

    if len(roots) >= 2:
        print("\nCutoff convergence:")
        previous_shift = None
        for (c_prev, r_prev), (c_now, r_now) in zip(roots, roots[1:]):
            shift = r_now - r_prev
            print(
                f"  {c_prev:,} -> {c_now:,}: "
                f"root shift {shift:+,.0f}"
            )
            if previous_shift is not None and previous_shift != 0:
                print(
                    f"    |new shift / previous shift| = "
                    f"{abs(shift / previous_shift):.4f}"
                )
            previous_shift = shift

        last_shift = abs(roots[-1][1] - roots[-2][1])
        print(f"\nlatest cutoff sensitivity:    {last_shift:,.0f}")
        print(
            "Suggested numerical report:     "
            f"n ~= {central:,.0f} "
            f"(latest cutoff shift about {last_shift:,.0f})"
        )
