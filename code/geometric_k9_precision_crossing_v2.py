#!/usr/bin/env python3
"""
geometric_k9_precision_crossing.py

Locate the numerical crossing of the ordinary ninth-order geometric
Bonferroni main term

    M^geom_{9,2}(n) = E1 - E2 + E3 - E4 + E5 - E6 + E7 - E8 + E9,
...
* the nine weighted prime measures are binned on a fine normalized
  logarithmic grid;
...
Recommended run
---------------
python geometric_k9_precision_crossing.py \
  --bracket 3.1e25,3.35e25 \
  --cutoffs 1e8,2e8,4e8 \
  --grids 65536,131072,262144 \
  --out k9_precision_crossing.csv
...
def geometric_moments(hist: np.ndarray) -> np.ndarray:
    """
    Compute E_1,...,E_9 from the binned prime measures using the
...
def M9_from_hist(hist: np.ndarray) -> tuple[float, np.ndarray]:
...
def main() -> None:
    ...
    ap.add_argument(
        "--out",
        type=Path,
        default=Path("k9_precision_crossing.csv"),
    )
    args = ap.parse_args()
    ...
    if len(cutoffs) < 1:
        raise SystemExit("--cutoffs requires at least one value")
