...
Examples
--------
Single shell (n is the square-root shell index):

    python multiplication_overlap_hierarchy.py --n 10000

Equivalent target specifications for the 12-fold primorial transition:

    python multiplication_overlap_hierarchy.py --value 152125131763605
    python multiplication_overlap_hierarchy.py --shell-lo 152125113877801

Both resolve to n=12333901.
...
def parse_args() -> argparse.Namespace:
    ...
    g.add_argument(
        "--n",
        type=int,
        help="analyze shell index n, meaning n^2 < m < (n+1)^2",
    )
    g.add_argument(
        "--value",
        type=int,
        help=(
            "analyze the square shell containing this integer value m; "
            "the shell index is floor(sqrt(m))"
        ),
    )
    g.add_argument(
        "--shell-lo",
        type=int,
        help="analyze the shell whose lower boundary is this perfect square n^2",
    )
...
def main() -> None:
    ...
    if args.n is not None:
        ...
        # Catch the common mistake of passing n^2 instead of n.
        root = isqrt(args.n)
        if args.n >= 10**10 and root * root == args.n:
            raise SystemExit(
                f"--n expects the shell INDEX, not the square boundary.\n"
                f"You supplied the perfect square {args.n} = {root}^2.\n"
                f"Use either:\n"
                f"  --n {root}\n"
                f"or:\n"
                f"  --shell-lo {args.n}"
        ...
        if args.value < 4:
            raise SystemExit("--value must be >= 4")
        single_n = isqrt(args.value)
        if single_n * single_n == args.value:
            raise SystemExit(
                f"--value {args.value} is exactly the square {single_n}^2, "
                f"so it lies on a shell boundary. "
                f"Use --shell-lo {args.value} to analyze the shell above it."
            )

    elif args.shell_lo is not None:
        if args.shell_lo < 4:
            raise SystemExit("--shell-lo must be >= 4")
        root = isqrt(args.shell_lo)
        if root * root != args.shell_lo:
            raise SystemExit(
                f"--shell-lo must be a perfect square; {args.shell_lo} is not."
            )
        single_n = root
    ...
    try:
        ...
        raise SystemExit(
            f"Could not allocate the row sieve for n={stop:,}.\n"
            f"Remember that --n is the shell index, not n^2. "
            f"For a square boundary use --shell-lo; for an integer inside "
            f"the shell use --value."
        )
    ...
    if single_n is not None:
        try:
            stats, hist = analyze_shell(single_n, rows, args.max_order)
        except MemoryError:
            raise SystemExit(
                f"Could not allocate the shell-depth array for n={single_n:,}.\n"
                f"This computation uses about n bytes for the depth array, "
                f"plus the row sieve and Python overhead."
            )

        if verification and single_n in verification:
            expected = verification[single_n]
            if stats["exact_holes"] != expected:
                raise AssertionError(
                    f"verification failed at n={single_n}: "
                    f"{stats['exact_holes']} != {expected}"
                )

        if args.value is not None:
            print(
                f"Resolved value {args.value} to shell index n={single_n} "
                f"({single_n**2} < m < {(single_n+1)**2})."
            )
        elif args.shell_lo is not None:
            print(
                f"Resolved lower square boundary {args.shell_lo} "
                f"to shell index n={single_n}."
            )
